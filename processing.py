...
def p300Processing2(filePath):
    ...
    rawData = pd.read_csv(RAWDATA_FILENAME, sep="\t", encoding='cp949')     # read Rawdata.txt
    extractData = rawData.loc[:, ['Time', 'EEG_Fp2']]   # extract 'Time', 'EEG_Fp2'
    ...
    # 비교결과 max에 해당하는 position을 return
    if max([left, right, up, down]) == left:
        print("left")
        return 'left'
    elif max([left, right, up, down]) == right:
