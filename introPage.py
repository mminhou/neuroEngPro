...
class IntroPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        '''else:
            self.userId = tk.Entry(self)
            self.canvas.create_window(400, 650, window=self.userId)
            self.userPw = tk.Entry(self)
            self.canvas.create_window(400, 700, window=self.userPw)
            # Intro Start Button
            self.startBtn = tk.PhotoImage(file='factory/image/startButton.png')
            startButton = tk.Button(self, image=self.startBtn, borderwidth=0, highlightthickness=0,
                                    command=lambda: self.getUser())
            self.canvas.create_window(1030, 700, window=startButton)'''
    # GIF animation
